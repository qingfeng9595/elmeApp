    ...
    <div class="foods-wrapper" ref="foodsWrapper">
      <ul>
        <li v-for="(item,index) in goods" :key="index" class="food-list food-list-hook">
          ...
          <ul>
            <li @click="selectFood(foods, $event)" v-for="(foods,index) in item.foods" :key="index" class="food-item">
              <div class="icon">
...
export default{
  ...
  created () {
    ...
    this.$http.get('/api/goods').then((response) => {
      ...
      if (response.errno === ERR_OK) {
        ...
        // 数据异步更新，初始BScroll时DOM无数据，无法获取DOM高度，故需$nextTick()才能正确计算滚动高度
        this.$nextTick(() => {
          this._initScroll();
  ...
  methods: {
    selectMenu (index, event) {
      // console.log(index);
      // 网页状态下触发默认点击事件，导致二次点击，目前网页测试无此事件，故取消。
